...
def k_fold_evaluation(X, y, k=5):
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(X)):
        ...
        model = SVR(kernel='rbf', gamma=0.1)  # Use RBF kernel with smaller gamma for finer Gaussian SVM
...
if __name__ == "__main__":
    ...
    df_results.to_excel("data/fine_gaussian_svm_results.xlsx", index=False, float_format="%.6f")

    # Save model
    dump(model, 'models/gaussian_svm_model.joblib')
