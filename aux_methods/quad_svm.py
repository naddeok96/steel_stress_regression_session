...
def k_fold_evaluation(X, y, k=5):
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(X)):
        ...
        model = SVR(kernel='poly', degree=2)  # Use polynomial kernel with degree=2 for quad SVM
...
if __name__ == "__main__":
    ...
    df_results.to_excel("data/quad_svm_results.xlsx", index=False, float_format="%.6f")

    # Save model
    dump(model, 'models/quad_svm_model.joblib')
